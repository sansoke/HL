  ...
  std::ostream & out)
{
  ...
  {
    out << "jam_num_cent_freqs: ";
    rosidl_generator_traits::value_to_yaml(msg.jam_num_cent_freqs, out);
    out << ", ";
  }

  // member: jam_state_cent_freqs
  {
    if (msg.jam_state_cent_freqs.size() == 0) {
      out << "jam_state_cent_freqs: []";
    } else {
      out << "jam_state_cent_freqs: [";
      size_t pending_items = msg.jam_state_cent_freqs.size();
      for (auto item : msg.jam_state_cent_freqs) {
